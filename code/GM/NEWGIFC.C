...
static table tab[4096];             /*  STRING TABLE!  */
...
int imagex=320,imagey=200;          /* dimensions of image. codesize-start val. of codestart */
...
int intro(FILE *gif,RGBdata *colors)
 {
  ...
  char header[7] = { 0,0,0,0,0,0,0 }; /* will contain GIF file header */
...
//    errorbox("There is no image in this file!","(R)eturn to Menu");
  ...
  codesize = fgetc(gif);      /*  number of bits to read for _trans */
                                  /*      after initializing table      */
  return(bkcol);
  }
...
int DrawGif(char *fname,RGBdata *colors)
  ...
  if ((fipo=fopen(fname,"rb"))==NULL) /* now bkdname contains GIF file name */
    return(FALSE); 
