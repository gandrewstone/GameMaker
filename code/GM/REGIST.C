...
// regist.C       Registration 2.0 registers PLAYGAME.EXE |
...
static char GetSerialNum(unsigned long int *num,char drive);
...
void main(void)
 {
  ...
  do
    ...
    do
      ...
      gotoxy(13,10);  printf("บ GM V"GMVER" Registration  บ");
...
static char GetSerialNum(unsigned long int *num,char drive)
  {
  char name[]="a:playgame.reg";
  char temp[31];
  FILE *fp;
  name[0]=drive;
  *num=0;
  if ( (fp=fopen(name,"rt")) == NULL) return(0);  // Missing File.
  fgets(temp,30,fp);                              // Get the user name.
  fscanf(fp,"%lu\n",num);   // Get serial number
  fclose(fp);
  return(1);
  }
